...
def softmax(P_hat):
    max_val = np.max(P_hat, axis=1).reshape(-1,1)
    numerator = np.exp(P_hat-max_val)
...
def Quantizer_v1(noisy_y, nb_z_classes, db_set):
    print('This Quantizer is designed for the case in which number of db is same :', nb_z_classes-1 == len(db_set))
    ...
    z_tmp[np.where( z_tmp < db_set[0] )[0]] = bs[0]  # 왼쪽 끝 대입
    z_tmp[np.where( z_tmp > db_set[nb_z_classes - 2] )[0]] = bs[nb_z_classes - 1] # 오른쪽 끝 대입

    for i in range(nb_z_classes-2): #0,1 (왼쪽이 기준)
        z_tmp[np.where( (db_set[i] <= z_tmp) & (z_tmp <= db_set[i+1]) )[0]] = bs[i+1]
